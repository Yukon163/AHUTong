...
object AHURepository {
    ...
    suspend fun getSchedule(isRefresh: Boolean = false) = withContext(Dispatchers.IO) {
        ...
        if (!isRefresh) {
            val localData = AHUCache.getSchedule("2023-2024", "2").orEmpty()
            if (localData.isNotEmpty()) {
                print("从本地获取课表")
                return@withContext Result.success(localData)
            }
        ...
        try {
            ...
            if (response.isSuccessful) {
                AHUCache.saveSchedule("2023-2024", "2", response.data)
                Log.e(TAG, "getSchedule: ${response.data}")
                Result.success(response.data)
...
//                            result.code = 0
//                            result.data =
//                                User(info.`object`.user.userName, info.`object`.user.idNumber)
//                            return@withContext result
//                        } else {
//                            result.code = -1
...
//                result
//            } catch (e: Throwable) {
//                Log.e(TAG, "loginWithCrawler: ${e.toString()}")
